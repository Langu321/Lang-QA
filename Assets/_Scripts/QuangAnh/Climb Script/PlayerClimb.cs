...
public class PlayerClimb : MonoBehaviour
{
    ...
    {
        if (playerState == PlayerState.NormalState) //bạn điều khiển nhân vật bằng input + CharacterController.
        {
        ...
        if (playerState == PlayerState.ClimbingState) //bạn không điều khiển bằng input, mà để animation tự di chuyển nhờ root motion.
        {
    ...
    private void Inputs()
    {
        ...
        if (Input.GetKeyDown(KeyCode.C)) // Pressing Button
        {
            ...
            {
                StartCoroutine(DropLedge()); // dropdown from ledge
            }
        }
    }
    ...
    {
        if (!isClimbing && thirdPersonController.Grounded) // if player is not climbing and is on the ground
        {
            for (int i = 0; i < rayAmount; i++) //create drawray line to check ledge
            {
    ...
    private void OnDrawGizmos()
    {
        ...
        // Vẽ vị trí tay trái (nếu animator có rig)
        if (animator != null)
        {
            ...
            if (leftHandBone != null)
            {
                ...
                Gizmos.DrawSphere(leftHandBone.position, 0.05f);

                // Vẽ đường nối từ tay -> ledge
                if (rayLedgeDownHit.point != Vector3.zero)
                {
                    Gizmos.color = Color.green;
                    Gizmos.DrawLine(leftHandBone.position, rayLedgeDownHit.point);
                }
            }
        }
    }
